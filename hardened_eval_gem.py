"""
Date of Creation: September 6, 2026, 7:00 PM
Path: C:\Users\loweb\AI_Financial_Sims\Gemini_ANNs\hardened_eval_gem.py
Original Legacy Source: C:\Users\loweb\AI_Financial_Sims\HO\HO_train_phase_ANNs\hardened_eval.py

Annotations:
- This is the robust end-to-end evaluator and runner script rewritten for the Gemini_ANNs project root directory.
- Automates the full training and inference sequence via controlled subprocesses while logging stdout and stderr streams.
- Performs rigorous prediction validation, naive baseline generation (with multi-ticker group awareness), metric computation (MAE, RMSE, MAPE), and statistical hypothesis testing (paired t-test or bootstrap).
- Generates final evaluation reports, structured manifests, and strict pass/fail exit criteria for pipeline orchestration.
"""
...
# ----------------------------
def main(argv):
    parser = argparse.ArgumentParser(description="Hardened evaluation runner (Gemini version).")
    ...
    # 4) BUILD NAIVE BASELINE (one-step lag of Actual_Close, group-safe)
    df = df.copy()
    ...
    diffs = abs_err_naive - abs_err_model
